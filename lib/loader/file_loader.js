...
class FileLoader {
   ...
   * @param {st set from ~/.vimrc
Press ENTER or type command to continueString|Function} options.caseStyle - set property's case when converting a filepath to property list.
   */
  // :bm, inject - 通过nodejs required之后, if export is a function type, then export will be called with inject as it param
  // then use what returns as exports
  constructor(options) {
   ...
   * @since 1.0.0
   */
  // 解析directory 下面的文件, 生成 props + export 的对象, 便于后面注入到 target 下面
  parse() {
    ...
    for (const directory of directories) {
      // :bm, 用 blobby 读出directory 下面所有的js和ts
