...
function sequence(tasks, names, results, missing, recursive, nest, optional, parent) {
  names.forEach(function(name) {
    ...
    // circular deps test. if parent is in proccess, then deps should rely on any of its parent
    } else if (nest.includes(name)) { 
      nest.push(name); // :bm, 这push和pop玩呢?!
    ...
    // recursive call 终止节点, 这个 name 的task必须执行
    if (!optional) {
...
module.exports = function(tasks, names) {
  ...
  return {
    sequence: results.sequence.filter(item => results.requires[item]), // 拿出需要执行的任务
    missingTasks: missing, // 丢失的
    recursiveDependencies: recursive, // cycular reference 的type节点
  };
};
